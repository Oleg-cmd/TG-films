...
import { sendKinoLink } from "./utils.js";

export const handleStartCommand = (ctx: any) => {
...
export const handleHelpCommand = (ctx: any) => {
  ctx.reply(
    `Available commands:\n` +
      `/start - Start the bot\n` +
...
export const handleKinoCommand = async (ctx: any) => {
  const telegramId = ctx.from.id.toString();
  try {
    ...
    ctx.reply(
      `✨ You are one step away from opening the app! ✨\n` +
